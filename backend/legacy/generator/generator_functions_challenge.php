...
function r_get_next_team(&$team) {
    ...
    if ($team == 0) {
        if (gp("r_need_volunteer")) {
            $team = gp("c_teams") + 1; // Volunteer team
        } else {
            $team = gp("c_teams");
        }
    }
...
function r_add_match($round, $match, $team_1, $team_2, $table_1 = 1, $table_2 = 2) {
    ...
    $r_match_plan[] = [
        ...
        'team_1' => ($team_1 > gp("c_teams")) ? 0 : $team_1,      // Change volunteer from gp("c_teams")+1 to 0
        'team_2' => ($team_2 > gp("c_teams")) ? 0 : $team_2,      // Change volunteer from gp("c_teams")+1 to 0
    ];
}
    ...
    foreach ($r_match_plan as $item) {
        if ($item['round'] == 1 && $item['match'] == $m1) {
            ...
            $new_match = [
                ...
                'team_1' => ($item['team_1'] > gp("c_teams")) ? 0 : $item['team_1'],      // Copy team_1 from the existing match. Volunteer not needed.
                'team_2' => ($item['team_2'] > gp("c_teams")) ? 0 : $item['team_2'],      // Copy team_2 from the existing match. Volunteer not needed.
            ];
...
function r_create_match_plan() {
    ...
    for ($round = 0; $round <= 3; $round++) {
        ...
        for($match = gp("r_matches_per_round"); $match >= 1; $match-- ) {
    ...
    if (gp("j_rounds") == 4) {
        $r_shift = ceil(gp("j_lanes") / 2);                  // TODO old code treated one lane differently
    } else {
        $r_shift = gp("j_lanes");
    }
    ...
    for ($match = 1; $match <= gp("r_matches_per_round"); $match++) {
        ...
        //
        // The issue only happens if gp("r_asym") is true
        // This means gp("c_teams") = 10, 14, 18, 22 or 26 teams (or one team less)
        //
        // gp("c_teams")  gp("j_rounds")  gp("j_lanes")   Match to add the empty game
        //   10         5        2            3
        //   14         5        3            4
        //   18         6        3            4
        //   18         5        4            5
        ...
        if (gp("r_asym") && $match == gp("j_lanes") + 1) {
        ...
        if ($match - $r_shift > 0) {
            ...
            r_add_test_match($match + $r_empty_match, $match - $r_shift + gp("r_matches_per_round"));
        }
    ...
    if ($DEBUG_RG) {
        ...
        foreach ($team_assignments as $team => $assignments) {
            ...
            if ($team >= 1 && $team <= gp("c_teams")) {
                ...
                if (gp("r_tables") == 2 && count($unique_tables) == 2) {
                    $color = '#d4edda'; // Green if both tables are used
                } elseif (gp("r_tables") == 4) {
    ...
    if ( (gp("c_teams") == 11 || gp("c_teams") == 12) && gp("j_lanes") == 3) {
...
// The time provide to the function is the start time of the match, regardless of robot check.
    ...
    if ($robot_check) {
        ...
        db_insert_activity(ID_ATD_R_CHECK, $time, gp('r_duration_robot_check'), 0, 0, $table_1, $team_1, $table_2, $team_2);
        g_add_minutes($time, gp('r_duration_robot_check'));
...
function r_insert_one_round($round) {
    ...
    foreach ($filtered_matches as $match) {
        ...
        if ($round == 0) {
            ...
            // Test round
            $duration = gp("r_duration_test_match");

        } else {

            // RG1 to RG3
            $duration = gp("r_duration_match");
        }
        ...
        if (!($match['team_1'] == 0 && $match['team_2'] == 0)) {
            ...
            r_insert_one_match($r_time, $duration, $match['table_1'], $match['team_1'], $match['table_2'], $match['team_2'], gp("r_robot_check"));
        }
        ...
        if (gp("r_tables") == 2) {              
            ...
            // In test round with four tables, match starts alternate between 5 and 10 minutes
                ...
                if($match['match'] % 2 == 1) {
                    ...
                    g_add_minutes($r_time, $duration - gp("r_duration_next_start"));
                }
    ...
    if (gp("r_robot_check")) {
        g_add_minutes($r_time, -1 * gp("r_duration_robot_check"));
    }
    ...
    switch ($round) {
        case 0:
            ...
            break;
        case 1:
            if ( gp("e_mode") == ID_E_MORNING || gp("e_mode") == ID_E_AFTERNOON ) {
...
function c_briefings($t, $c_day) {           
    ...
    if ($c_day == 1) {
        ...
        db_insert_activity(ID_ATD_C_COACH_BRIEFING, g_shift_minutes($t, -1 * (gp("c_duration_briefing") + gp("c_ready_opening"))), gp("c_duration_briefing"));    
    ...
    if (! gp("j_briefing_after_opening")) {

        db_insert_activity(ID_ATD_C_JUDGE_BRIEFING, g_shift_minutes($t, -1 * (gp("j_duration_briefing") + gp("c_ready_opening"))), gp("j_duration_briefing"));    

    } else {

        g_add_minutes($j_time, gp("j_ready_briefing"));
        db_insert_activity(ID_ATD_C_JUDGE_BRIEFING, $j_time, gp("j_duration_briefing")); 
        
        // move time forward
        g_add_minutes($j_time, gp("j_duration_briefing"));   
    ...
    if (! gp("r_briefing_after_opening")) {

        if ($c_day == 1) {
            // One day event: Full briefing
            db_insert_activity(ID_ATD_R_REFEREE_BRIEFING, g_shift_minutes($t, -1 * (gp("r_duration_briefing") + gp("c_ready_opening"))), gp("r_duration_briefing"));    
        } else {
            // Second day of the event: Short briefing
            db_insert_activity(ID_ATD_R_REFEREE_BRIEFING, g_shift_minutes($t, -1 * (gp("r_duration_briefing_2") + gp("c_ready_opening"))), gp("r_duration_briefing_2"));    
        }

    } else {

        g_add_minutes($r_time, gp("r_ready_briefing"));

        if ($c_day == 1) {
            // One day event: Full briefing
            db_insert_activity(ID_ATD_R_REFEREE_BRIEFING, $r_time, gp("r_duration_briefing"));  
            g_add_minutes($r_time, gp("r_duration_briefing"));  
        } else {
            // Second day of the event: Short briefing
            db_insert_activity(ID_ATD_R_REFEREE_BRIEFING, $r_time, gp("r_duration_briefing_2"));
            g_add_minutes($r_time, gp("r_duration_briefing_2"));
        }

    }

    // Buffer between opening (or briefing respectively) and first action for teams and judges
    g_add_minutes($j_time, gp("j_ready_action"));
    g_add_minutes($r_time, gp("r_ready_action"));
...
function r_final_round($team_count) {
    ...
    switch($team_count) {
        ...
        case 16:
            ...
            r_insert_one_match($r_time, gp("r_duration_match"), 1, 0, 2, 0, gp("r_robot_check_16"));
            g_add_minutes($r_time, gp("r_duration_next_start"));

            r_insert_one_match($r_time, gp("r_duration_match"), 3, 0, 4, 0, gp("r_robot_check_16"));
            g_add_minutes($r_time, gp("r_duration_next_start"));
            ...
            r_insert_one_match($r_time, gp("r_duration_match"), 3, 0, 4, 0, gp("r_robot_check_16"));
            g_add_minutes($r_time, gp("r_duration_match"));
        ...
        case 8:    
            ...
            if (gp("r_tables") == 2) {
                ...
                r_insert_one_match($r_time, gp("r_duration_match"), 1, 0, 2, 0, gp("r_robot_check_8"));
                g_add_minutes($r_time, gp("r_duration_match"));
                ...
                r_insert_one_match($r_time, gp("r_duration_match"), 3, 0, 4, 0, gp("r_robot_check_8"));
                g_add_minutes($r_time, gp("r_duration_next_start"));
        ...
        case 4:
            ...
            if (gp("r_quarter_final")) {
                ...
                if (gp("r_tables") == 2) {
                    ...
                    r_insert_one_match($r_time, gp("r_duration_match"), 1, 0, 2, 0, gp("r_robot_check_4"));    
                    g_add_minutes($r_time, gp("r_duration_match"));
                    ...
                    r_insert_one_match($r_time, gp("r_duration_match"), 3, 0, 4, 0, gp("r_robot_check_4"));    
                    g_add_minutes($r_time, gp("r_duration_match"));
        ...
        case 2:
            ...
            r_insert_one_match($r_time, gp("r_duration_match"), 1, 0, 2, 0, gp("r_robot_check_2"));
            g_add_minutes($r_time, gp("r_duration_match"));
            ...
            r_insert_one_match($r_time, gp("r_duration_match"), 1, 0, 2, 0, false); // only match without robot check
            g_add_minutes($r_time, gp("r_duration_match"));
 ...
 function j_judging_one_round($c_block, $j_t) {
    ...
    for ($j_l = 1; $j_l <= gp("j_lanes"); $j_l++) {
        ...
        if ($j_t + $j_l <= gp("c_teams")) {
            ...
            db_insert_activity(ID_ATD_C_WITH_TEAM, $j_time, gp("j_duration_with_team"), $j_l, $j_t + $j_l, 0, 0, 0, 0);
            
        }
    }
    g_add_minutes($j_time, gp("j_duration_with_team"));
    ...
    for ($j_l = 1; $j_l <= gp("j_lanes"); $j_l++) {
        ...
        if ($j_t + $j_l <= gp("c_teams")) {
            ...
            db_insert_activity(ID_ATD_C_SCORING, $j_time, gp("j_duration_scoring"), $j_l, $j_t + $j_l, 0, 0, 0, 0);
        }
    }
    g_add_minutes($j_time, gp("j_duration_scoring"));

    // breaks before NEXT round
    if ( (gp("j_rounds") == 4 && $c_block == 2) ||
         (gp("j_rounds") > 4 && $c_block == 3) ) {
        // lunch break
        g_add_minutes($j_time, gp("j_duration_lunch"));
    } else {
        // normal break, but not after final block
        if ($c_block < gp("j_rounds")) {
            g_add_minutes($j_time, gp("j_duration_break"));
        }
    }
