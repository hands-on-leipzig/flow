...
final class PreviewGridOverlapResolver
{
    ...
    /**
     * @param  list<array{column_key: string, start: Carbon, end: Carbon, text: string, rowspan: int, style_column: string, activity_id?: int}>  $placed
    ...
    public static function resolve(array $placed): array
    {
        ...
        $placed = self::dedupeIdenticalPlacements($placed);
            ...
            foreach ($columnEvents as $event) {
                ...
                foreach ($earlier as $a) {
                    ...
                    if ($b['start']->equalTo($a['start'])) {
                        $sameStartPartner ??= $a;
                    } elseif ($a['start']->lt($b['start']) && self::gridEnd($b)->lte(self::gridEnd($a))) {
                        $containedPartner ??= $a;
                    } else {
                        $partialPartner ??= $a;
                    }
                ...
                if ($sameStartPartner !== null) {
                    $b['start'] = self::gridEnd($sameStartPartner);
                ...
                if ($partialPartner !== null) {
                    $b['start'] = self::gridEnd($partialPartner);
                    $b['overlap_adjusted'] = true;
                    self::recalculateRowspan($b);
        ...
        foreach ($resolved as $event) {
            unset($event['_idx'], $event['activity_id']);
            $clean[] = $event;
        }
    ...
    /**
     * One activity can produce duplicate placement rows (e.g. table_1 === table_2).
     *
     * @param  list<array{column_key: string, start: Carbon, text: string, activity_id?: int}>  $placed
     * @return list<array>
     */
    public static function dedupeIdenticalPlacements(array $placed): array
    {
        $seen = [];
        $deduped = [];

        foreach ($placed as $event) {
            $activityId = (int) ($event['activity_id'] ?? 0);
    ...
    /**
     * Half-open grid occupancy: [start, gridEnd).
     *
     * @param  array{start: Carbon, end: Carbon, rowspan: int}  $a
     * @param  array{start: Carbon, end: Carbon, rowspan: int}  $b
     */
    private static function intervalsOverlap(array $a, array $b): bool
    {
        return $a['start']->lt(self::gridEnd($b)) && $b['start']->lt(self::gridEnd($a));
    }

    /**
     * @param  array{start: Carbon, rowspan: int}  $event
     */
    private static function gridEnd(array $event): Carbon
    {
        return $event['start']->copy()->addMinutes((int) $event['rowspan'] * self::SLOT_MINUTES);
    }
