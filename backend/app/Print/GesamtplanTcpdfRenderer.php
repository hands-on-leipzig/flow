...
final class GesamtplanTcpdfRenderer
{
    ...
    {
        $pdf = EventPrintPdf::make('Gesamtplan');
        ...
        if ($sections === []) {
            ...
            $pdf->sectionSubject = 'Gesamtplan';
            $pdf->AddPage();
        }

        foreach ($sections as $section) {
            $pdf->colorHex = (string) ($section['color_hex'] ?? EventPrintPdf::HOT_ORANGE);
            $pdf->sectionSubject = (string) ($section['subject'] ?? 'Gesamtplan');
            $pdf->noshowSubject = false;
            $pdf->logoPath = null;
            $pdf->AddPage();

            $ablauf = $section['ablauf'] ?? [];
            if (is_array($ablauf) && $ablauf !== []) {
                $this->table($pdf, 'Ablauf', $ablauf);
            }
