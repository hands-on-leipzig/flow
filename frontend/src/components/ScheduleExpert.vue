  ...
  <div class="schedule-expert flex flex-col gap-4 pb-2">
    <ProgramSection program="explore" :active="showExplore">
      <div v-if="showExplore">
        <template v-for="(group, programName) in expertParamsGrouped" :key="programName">
          <template v-if="String(programName).toLowerCase().includes('explore')">
      ...
      <p v-else class="program-empty">
        Ausgeschaltet unter Allgemein — dort aktivieren, um Expertenparameter zu setzen.
      </p>
    </ProgramSection>
    ...
    <ProgramSection program="challenge" :active="showChallenge">
      <div v-if="showChallenge">
        <template v-for="(group, programName) in expertParamsGrouped" :key="programName">
          <template v-if="String(programName).toLowerCase().includes('challenge')">
            <template v-for="param in group" :key="param.id">
              <ParameterField
                  v-if="visibilityMap[param.id]"
                  :param="param"
        ...
        <div class="mt-3 md:mt-4 w-full max-w-lg">
